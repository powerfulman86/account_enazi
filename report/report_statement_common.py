...
class ReportStatementCommon(models.AbstractModel):
    ...
    def _show_buckets_sql_q1(self, partners, date_end):
        return str(
            ...
            WHERE l.partner_id IN %(partners)s
                                AND l.account_internal_type in ('receivable','payable')
                                AND (
    ...
    def _get_bucket_dates(self, date_end, aging_type):
        return getattr(self, "_get_bucket_dates_%s" % aging_type, self._get_bucket_dates_days)(date_end)
    ...
    def _get_account_show_buckets(self, company_id, partner_ids, date_end, aging_type):
    ...
    def _get_bucket_labels(self, date_end, aging_type):
        return getattr(self, "_get_bucket_labels_%s" % aging_type, self._get_bucket_dates_days)(date_end)
    ...
    def _get_line_currency_defaults(self, currency_id, currencies, balance_forward):
        ...
        due_to_words = ''
        due_to_words = self._compute_amount_words(balance_forward)
        return (
            {
                "lines": [],
                "buckets": [],
        ...
        for partner_id in partner_ids:
            ...
            for line in lines[partner_id]:
                ...
                if not line["blocked"]:
                    ...
                    line_currency["due_to_words"] = self._compute_amount_words(line_currency["amount_due"])
    ...
    def _compute_amount_words(self, close_balance):
        total_amount_words = ''
        ...
        try:
            ...
            subunit = self.env['ir.translation'].search([('lang', '=', lang.code),
                                                         ('src', '=',
                                                          currency.currency_subunit_label)], limit=1)
            if not unit:
            ...
            if unit and subunit and unit.value and subunit.value:
                ...
                elif amount or not close_balance:
                    total_amount_words = num2words(Decimal(str(amount)), lang='ar') + ' ' + \
                ...
                total_amount_words = num2words(Decimal(str(close_balance)), lang='ar') + ' ' + str(currency.symbol)
        except NotImplementedError:
            total_amount_words = num2words(Decimal(str(close_balance)), lang='en') + ' ' + str(
